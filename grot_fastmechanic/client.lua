        ...
        if perf.modType == 18 then
            available.performance[perf.id] = {
                ...
                options = {
                    ...
                    {value = true, label = 'Włączone'}
                }
            }
        else
            local options = {}
            table.insert(options, {value = -1, label = 'Stock'})
            ...
            for i = 0, levels - 1 do
                table.insert(options, {value = i, label = 'Poziom ' .. (i + 1)})
            end
            available.performance[perf.id] = {
                label = perf.label,
                modType = perf.modType,
            ...
            for i = 0, maxMod - 1 do
                ...
                if label and label ~= "NULL" and label ~= "" then
                    ...
                    if translatedLabel and translatedLabel ~= "NULL" and translatedLabel ~= label then
                        ...
                        table.insert(options, {value = i, label = 'Opcja ' .. (i + 1)})
                    end
                else
                    table.insert(options, {value = i, label = 'Opcja ' .. (i + 1)})
                end
...
function OpenMechanic()
    ...
    if vehicle == 0 then
        if GetResourceState('grot_notify') == 'started' then
            exports['grot_notify']:Notify('error', 'Nie jesteś w pojeździe!', 3000)
        end
        return
    end
                        ...
                        if dist < 20.0 then
                            ...
                            if dist < Config.InteractionDistance then
                                if not menuOpen then
                                    ...
                                    AddTextComponentSubstringPlayerName('Naciśnij ~b~E~w~ aby otworzyć warsztat mechanika')
    ...
    while true do
        ...
        if Config.EnableVehicleKey then
            if IsControlJustReleased(0, Config.OpenKey) then
                print('[FastMechanic] F6 naciśnięte')
                if menuOpen then
                    print('[FastMechanic] Zamykanie menu')
                    CloseMechanic()
                else
                    if IsPlayerInVehicle() then
                        print('[FastMechanic] Gracz w pojeździe, otwieranie menu')
                        OpenMechanic()
                    else
                        print('[FastMechanic] Gracz NIE jest w pojeździe')
                    end
                end
            end
        else
            if menuOpen and IsControlJustReleased(0, Config.OpenKey) then
                CloseMechanic()
            end
        end
        if menuOpen then
            BeginTextCommandDisplayHelp('STRING')
            AddTextComponentSubstringPlayerName('Naciśnij ~b~F6~w~ lub ~b~ESC~w~ aby zamknąć')
            EndTextCommandDisplayHelp(0, false, true, -1)
        end
    end
end)
